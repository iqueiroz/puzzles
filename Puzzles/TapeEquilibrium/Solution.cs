...
The difference between the two parts is the value of: |(A[0] + A[1] + ... + A[P − 1]) − (A[P] + A[P
+ 1] + ... + A[N − 1])|
...
namespace Puzzles.TapeEquilibrium
{
    using System;
    public class Solution
    {
        ...
        {
            int sum = sumArray(A);
            int returnValue = calculateMinimalDifference(A, sum);
            return returnValue;
        }

        private static int calculateMinimalDifference(int[] A, int sum)
        {
            var returnValue = 0;
            var part1 = sum;
            var part2 = 0;
            ...
            for (int i = lastIndex; i >= 1; i--)
            {
                part1 -= A[i];
                part2 += A[i];
                var difference = Math.Abs(part1 - part2);
                if (i == lastIndex || difference < returnValue)
